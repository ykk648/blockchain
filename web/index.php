...
?>
<div align="center">
<img  src="bg.png" width="1000px" height="500px" />
</div>
<h2 align="center">当前节点：<?php echo $_REQUEST["port"]?></h2>


<?php

//挖矿功能
if($_REQUEST["mine"]==1&&count($_REQUEST["my_message"])>0){
	$transdata='{"message":"'.$_REQUEST["my_message"].'"}';
...
if($_REQUEST["reslove"]==1){
	...
	$namelist[0]='God';
	$namevalue[0]=0;
	foreach((array)$item_chain->chain as $block){
...
<div align="center">
<a class="btn btn-primary" href="index.php?port=<?php echo $_REQUEST["port"]?>&chain=1" role="button">刷新链上消息</a>
...
<h2>消息计数</h2>
<?php
for($i=2;$i<count($namelist);$i++){
		echo "<br/>".$namelist[$i]."有".$namevalue[$i]."条消息";
	}
?>
</div>
